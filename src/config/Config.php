...
class Config{
    ...
    private function __construct(){
        ...
        $this->postgresHost = getenv('POSTGRES_HOT') ?? 'localhost';
        $this->postgresPort = getenv('POSTGRES_PORT') ?? '5432';
        $this->db = new PDO("pgsql:host={$this->postgresHost};port={$this->postgresPort};dbname={$this->postgres}", $this->postgresUser, $this->postgresPassword);
    }
